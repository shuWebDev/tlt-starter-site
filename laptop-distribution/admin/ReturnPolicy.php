...
if ( ! preg_match('/[0-9]{8}/', $studentID, $match) ) echo "huh?";
//else echo $studentID;
$thisStudent = $match[0];
...
if ( ! $statusResults = $link->prepare("SELECT formResponses.returnPolicy, students.sn, 
  students.fn, students.mi
  FROM formResponses LEFT JOIN students ON 
  formResponses.ID=students.ID WHERE formResponses.ID=?")) {
...
if ( ! $statusResults->bind_result($rp, $sn, $fn, $mi) ) {
...
<li> I understand that I must return the laptop to the Asset Management Office located in Corrigan Hall Room 27 no later than 4:00 pm on Thursday, August 4, 2016.  The hours of operation for the Asset Management Office are Monday – Thursday, 9:00 am – 4:00 pm and Friday, 9:00 am – 12:00 pm.</li>
...
<fieldset>
  <legend>Agreement</legend>
          ...
          <input type="text" disabled="disabled" class="form-control" value="<? echo "$fn ";
...
<button type="button" id="verification" class="access btn btn-lg btn-primary" disabled="disabled"><? 
      if ( '0000-00-00 00:00:00' != $rp ) echo 'Completed';
